...
export const usePhonebookSearchModule = (): {
  ...
  const [phonebookSearchModule, setPhonebookSearchModule] = useNethlinkData(
    'phonebookSearchModule',
  )
  const { NethVoiceAPI } = useLoggedNethVoiceAPI()
  const update =
    <T>(selector: keyof PhonebookSearchModuleData) =>
    (value: T | undefined) => {
  ...
  function prapareSearchData(receivedPhoneNumbers: SearchCallData) {
    receivedPhoneNumbers.rows = receivedPhoneNumbers.rows.map(
      (contact: SearchData) => {
  ...
  const searchPhonebookContacts = async () => {
    const searchResult: SearchCallData = await NethVoiceAPI.Phonebook.search(
      phonebookSearchModule?.searchText || '',
    )
  ...
  return {
    searchTextState: [
      phonebookSearchModule?.searchText,
      update<string | null>('searchText'),
    ] as StateType<string | null>,
