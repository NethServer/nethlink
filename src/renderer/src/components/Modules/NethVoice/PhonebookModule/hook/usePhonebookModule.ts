...
export const usePhonebookModule = (): {
  ...
  const [phonebookModule, setPhonebookModule] =
    useNethlinkData('phonebookModule')
  const { NethVoiceAPI } = useLoggedNethVoiceAPI()

  const update =
    <T>(selector: keyof PhonebookModuleData) =>
    (value: T | undefined) => {
  ...
  const handleAddContactToPhonebook = async (contact: ContactType) => {
    const result: ContactType =
      await NethVoiceAPI.Phonebook.createContact(contact)
    return result
  }

  return {
    selectedContact: [
      phonebookModule?.selectedContact,
      update<SelectedContact>('selectedContact'),
    ] as StateType<SelectedContact>,
