...
export const useSpeedDialsModule = (): {
  ...
  const [speeddialsModule, setSpeeddialsModule] =
    useNethlinkData('speeddialsModule')
  const [rawSpeedDials, setRawSpeedDials] = useNethlinkData('speeddials')
  const [speedDials, setSpeedDials] = useState<ContactType[] | undefined>(
    undefined,
  )
  const { NethVoiceAPI } = useLoggedNethVoiceAPI()
  const update =
    <T>(selector: keyof SpeedDialModuleData) =>
    (value: T | undefined) => {
  ...
  const deleteSpeedDial = async (speedDial) => {
    try {
      ...
      setRawSpeedDials((p) =>
        p?.filter((s) => s.id?.toString() !== speedDial.id?.toString()),
      )
    } catch (e) {
      throw new Error()
    }
  ...
  const upsertSpeedDial = async (
    speedDial: NewSpeedDialType | NewContactType,
  ) => {
    try {
      const selectedSpeedDial = speeddialsModule?.selectedSpeedDial
      if (selectedSpeedDial) {
        const updatedSpeedDial = await NethVoiceAPI.Phonebook.updateSpeeddial(
          speedDial,
          selectedSpeedDial,
        )
        if (updatedSpeedDial) {
          const newSpeedDials = rawSpeedDials?.map((speedDial) =>
            speedDial.id?.toString() === updatedSpeedDial['id']
              ? (updatedSpeedDial! as ContactType)
              : speedDial,
          )
        ...
        const newSpeedDial =
          await NethVoiceAPI.Phonebook.createSpeeddial(speedDial)
  ...
  return {
    speedDials,
    speedDialsState: [
      speeddialsModule?.selectedSpeedDial,
      update<ContactType>('selectedSpeedDial'),
    ] as StateType<ContactType>,
    favouriteState: [
      speeddialsModule?.selectedFavourite,
      update<ContactType>('selectedFavourite'),
    ] as StateType<ContactType>,
    deleteSpeedDial,
    upsertSpeedDial,
  }
