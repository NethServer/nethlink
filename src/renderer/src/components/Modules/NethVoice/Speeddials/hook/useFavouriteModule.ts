...
export const useFavouriteModule = () => {
  ...
  const [favourites, setFavourites] = useState<ContactType[] | undefined>(
    undefined,
  )
  ...
  const getUsernameFromContact = (contact: ContactType): string | undefined => {
    ...
    const usernameFromExtension = extension
      ? operators?.extensions?.[extension]?.username
      : undefined
  ...
  useEffect(() => {
    rawSpeedDials &&
      filterFavourites(
        rawSpeedDials,
        speeddialsModule?.favouriteOrder || FilterTypes.AZ,
      )
  }, [rawSpeedDials, speeddialsModule?.favouriteOrder, operators])

  const getSorter = (order: FilterTypes) => {
    let sorter: ((a: ContactType, b: ContactType) => number) | undefined =
      undefined
    switch (order) {
      case FilterTypes.ZA:
      ...
      case FilterTypes.EXT:
        sorter = (a: ContactType, b: ContactType) =>
          (a.speeddial_num || '') > (b.speeddial_num || '') ? 1 : -1
        break
      default:
        sorter = (a: ContactType, b: ContactType) =>
          (a.name || '') > (b.name || '') ? 1 : -1
  ...
  const isSearchAlsoAFavourite = (contact: SearchData) => {
    const foundedContact = favourites?.find(
      (s) => s.speeddial_num === (contact.speeddial_num || contact.extension),
    )
    if (foundedContact) {
  ...
  function toggleFavouriteFromSearch(contact: SearchData) {
    debouncer(
      `toggleFavourite-${contact.id}`,
      async () => {
        const speedDial = favourites?.find(
          (s) =>
            s.speeddial_num === (contact.speeddial_num || contact.extension),
        )
        if (!speedDial) {
