      ...
      <Scrollable>
        <div className='flex flex-col w-full items-center px-4 text-bgDark dark:text-bgLight'>
          <div className='mt-6 mb-4 flex flex-col items-center gap-2'>
            <img src={parseThemeToClassName(theme) === 'dark' ? NethLinkLogoSimpleDark : NethLinkLogoSimple} className="h-10 overflow-hidden object-cover place-items-start"></img>
            <div className='font-medium'>NethLink by <span className='text-textBlueLight dark:text-textBlueDark cursor-pointer' onClick={openNethesisPage}>Nethesis</span></div>
          </div>
          <div className='text-gray-400 mb-2'>{t('About.current_version', { version: page?.props.appVersion })}</div>
          {
            !!notifications?.system?.update && <div className='mt-6 flex flex-col gap-2 items-center'>
              <span className='font-medium text-[12px] leading-[18px]'>{t('About.update_available')}</span>
              <Button
                variant='white'
