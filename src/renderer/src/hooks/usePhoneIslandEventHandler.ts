...
export const usePhoneIslandEventHandler = () => {
  ...
  const [missedCalls, setMissedCalls] = useRefState(
    useNethlinkData('missedCalls'),
  )
  ...
  const onMainPresence = useCallback(
    (op: { [username: string]: any }) => {
  ...
  const gestLastCalls = (newLastCalls: { count: number; rows: CallData[] }) => {
    ...
    const existingIds = new Set(
      (lastCalls.current || []).map((c) => c.uniqueid),
    )
    const diff = newLastCalls.rows.filter((c) => !existingIds.has(c.uniqueid))

    if (diff.length > 0) {
      const newMissed = diff.filter(
        (c) => c.direction === 'in' && c.disposition === 'NO ANSWER',
      )

      newMissed.forEach((c) => {
        const differenceBetweenTimezone = diffValueConversation(
          getTimeDifference(account.current!, false),
        )
        const timeDiff = format(
          utcToZonedTime(c.time! * 1000, differenceBetweenTimezone),
          'HH:mm',
        )
        sendNotification(
          t('Notification.lost_call_title', {
            user: c.cnam || c.ccompany || c.src || t('Common.Unknown'),
          }),
          t('Notification.lost_call_body', {
            number: c.src,
            datetime: timeDiff,
          }),
        )
      ...
      if (newMissed.length > 0) {
        ...
        setMissedCalls((p) => {
          ...
          const uniqueNewMissed = newMissed.filter(
            (c) => !existingMissedIds.has(c.uniqueid),
          )
