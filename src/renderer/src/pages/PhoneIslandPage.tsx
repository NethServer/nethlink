...
import {
  usePhoneIslandEventListener,
  resetPhoneIslandReadyState,
} from '@renderer/hooks/usePhoneIslandEventListeners'
...
export function PhoneIslandPage() {
  ...
  useInitialize(() => {
    ...
    window.electron.receive(
      ...
      (devices: PreferredDevices & { shouldRunWarmup?: boolean }) => {
        const { shouldRunWarmup, audioInput, videoInput, audioOutput } = devices
        Log.info('Received CHANGE_PREFERRED_DEVICES in PhoneIslandPage:', {
          devices,
          shouldRunWarmup,
        })
        ...
        if (shouldRunWarmup) {
          Log.info(
            'Requesting audio warm-up from main process (first initialization)...',
          )
          setTimeout(() => {
          ...
          setTimeout(() => {
            eventDispatch(
              PHONE_ISLAND_EVENTS['phone-island-audio-input-change'],
              { deviceId: audioInput },
            )
            eventDispatch(
              PHONE_ISLAND_EVENTS['phone-island-video-input-change'],
              { deviceId: videoInput },
            )
            eventDispatch(
              PHONE_ISLAND_EVENTS['phone-island-audio-output-change'],
          ...
          Log.info(
            'Skipping audio warm-up (already done or reconnection), dispatching device changes immediately',
          )
          eventDispatch(
            PHONE_ISLAND_EVENTS['phone-island-audio-input-change'],
            { deviceId: audioInput },
    ...
    window.electron.receive(
      ...
      (settings: { ringtoneName: string; outputDeviceId: string }) => {
        Log.info(
          'Received CHANGE_RINGTONE_SETTINGS in PhoneIslandPage:',
          settings,
        )
        ...
        if (settings.ringtoneName) {
          eventDispatch(
            PHONE_ISLAND_EVENTS['phone-island-ringing-tone-select'],
            {
        ...
        if (settings.outputDeviceId) {
          eventDispatch(
            PHONE_ISLAND_EVENTS['phone-island-ringing-tone-output'],
            {
    ...
    window.electron.receive(
      ...
      (audioData: {
        ...
        eventDispatch(
          PHONE_ISLAND_EVENTS['phone-island-audio-player-start'],
          audioData,
        )
      },
    )
    ...
    window.electron.receive(IPC_EVENTS.PHONE_ISLAND_READY, () => {
      ...
      setTimeout(() => {
        try {
          ...
          const savedRingtoneRaw = localStorage.getItem(
            'phone-island-ringing-tone',
          )
          const savedOutputDeviceRaw = localStorage.getItem(
          ...
          if (ringtoneName || outputDeviceId) {
            Log.info('Applying saved ringtone settings on startup:', {
              ringtoneName,
              outputDeviceId,
            })
    ...
    window.electron.receive(
      ...
      async (deviceInformationObject, force) => {
        Log.debug('CHANGE_DEFAULT_DEVICE', { force, deviceInformationObject })
        const changed = await NethVoiceAPI.User.default_device(
  ...
  const resize = (phoneIsalndSize: PhoneIslandSizes) => {
    if (!isOnLogout.current) {
      const { width, height, top, bottom, left, right, bottomTranscription } =
        phoneIsalndSize.sizes
      const w = Number(width.replace('px', ''))
      const h = Number(height.replace('px', ''))
      const r = Number((right ?? '0px').replace('px', ''))
      const transcription = Number(
        (bottomTranscription ?? '0px').replace('px', ''),
      )
      ...
      phoneIslandContainer.current?.setAttribute(
        'style',
        `
      ...
      innerPIContainer.current?.setAttribute(
        'style',
        `
  ...
  useEffect(() => {
    if (account && !isDataConfigCreated.current) {
      ...
      createDataConfig(account)
        .then(([deviceInformation, dataConfig]) => {
  ...
  useEffect(() => {
    if (
      account &&
      Object.keys(eventsRef.current).length > 0 &&
      !attachedListener.current
    ) {
      Log.debug(
        account?.username,
        'attachd listeners',
        Object.keys(eventsRef.current).length,
      )
      Object.entries(eventsRef.current).forEach(
        ([phoneIslandEventName, callback]) => {
  ...
  async function logout() {
    ...
    if (deviceInformationObject.current) {
      ...
      await dispatchAndWait(
        PHONE_ISLAND_EVENTS['phone-island-detach'],
        PHONE_ISLAND_EVENTS['phone-island-detached'],
        {
          data: {
  ...
  return (
    <div ref={phoneIslandContainer} id={'phone-island-container'}>
      <div
        style={{
          position: 'absolute',
          height: '100vh',
      ...
      <ElectronDraggableWindow>
        <div
          ref={innerPIContainer}
          id='phone-island-inner-container'
          className='relative w-full h-full'
        >
          {account && (
            <PhoneIslandContainer
              dataConfig={dataConfig}
              deviceInformationObject={deviceInformationObject.current}
              isDataConfigCreated={isDataConfigCreated.current}
            />
          )}
        </div>
      </ElectronDraggableWindow>
