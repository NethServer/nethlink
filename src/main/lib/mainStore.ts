...
class Store<T> {
  ...
  constructor() {
    ...
    if (
      !fs.existsSync(this.USER_DATA_PATH) ||
      !fs.existsSync(AVAILABLE_USER_DATA_PATH)
    ) {
        ...
        if (!fs.existsSync(this.USER_DATA_PATH)) {
          ...
          if (retrivedStore.auth?.availableAccounts) {
            fs.writeFileSync(
              ...
              JSON.stringify(retrivedStore.auth!.availableAccounts),
            )
          } else {
            fs.writeFileSync(AVAILABLE_USER_DATA_PATH, JSON.stringify({}))
          }
      ...
      !fs.existsSync(this.USER_DATA_PATH) &&
        fs.writeFileSync(this.USER_DATA_PATH, JSON.stringify({}))
    } else {
  ...
  getAvailableFromDisk(): { [accountUID: string]: Account } {
    if (fs.existsSync(AVAILABLE_USER_DATA_PATH)) {
      const availableUserData = fs.readFileSync(
        AVAILABLE_USER_DATA_PATH,
        'utf-8',
      )
  ...
  getFromDisk() {
    try {
      ...
      ;(retrivedStore as LocalStorageData).auth!.availableAccounts =
        JSON.parse(availableUserData)
